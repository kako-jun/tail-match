...
- **🚨 status（譲渡済み）フィールド未実装問題** (2025-11-12監査):
  - **14施設でstatusフィールドが未設定** - yaml-to-db.jsのデフォルト値に依存
  - **19施設で譲渡済み判定が未実装** - 沖縄県のみが実装済み
  - 影響: 譲渡済み動物が「available」として表示されている可能性（深刻）
  - 詳細: 全20施設の監査結果を記録（下記参照）
- **🚨 フォールバック抽出問題**:
...
### 全施設スクレイパー監査 (2025-11-12) 🔍

**調査対象**: 全20施設のhtml-to-yaml.js
...
| ------------ | -------- | -------- | -------- |
| animal_type  | ✅ 17/20 | -        | -        |
| status設定   | 6/20     | ⚠️ 14/20 | -        |
| 譲渡済み判定 | 1/20     | -        | 🚨 19/20 |

#### 推奨される対応

1. **優先度1**: statusフィールドを全施設に追加（最低限 `status: 'available'`）
2. **優先度2**: 各施設のHTMLで譲渡済みマーカーを調査し、判定ロジックを実装
   - **個別パターン**: 「※譲渡しました」などの文言が各動物に付く
   - **見出しパターン**: セクション全体が「譲渡済み動物」として分離
   - 両パターンに対応する必要あり
3. **優先度3**: 犬用ページの追加（7施設）
