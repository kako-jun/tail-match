...
- **[保護施設データベース](./.claude/shelters.yaml)** - 全77施設の情報（実装済み26＋未実装51）
...
- **共通ヘルパー関数**: 全30施設で統一ロジックを使用（譲渡済み判定・動物種判定）
  - 詳細は [スクレイピングアーキテクチャ](./.claude/scraping-architecture.md#汎用化完了) 参照
- **命名規則統一**: -cats/-dogs サフィックス完備（猫専用21 + 犬専用11 + 混在4）
  - 詳細は [実装ガイド](./.claude/scraping-guide.md#命名規則) 参照
- **犬用ページ実装**: 11施設で犬用スクレイパーを追加実装
...
- **全施設ロガー統合**:
  - 30施設すべての html-to-yaml.js に logger.start() / loadPreviousCounts() / finalize() を統合
...
**Phase 4.2**: 複数自治体対応 🔄 **進行中** - 13都道府県、26施設、34スクレイパー実装済み
...
**徳島県2施設の実装完了（2025-11-15）:**

1. **徳島県動物愛護管理センター（猫）** `tokushima/tokushima-pref-cats`
   - iframe内テーブル形式（table.f_a3）、rowspan対応パース処理、全角数字→半角変換実装、データ: 1匹

2. **徳島県動物愛護管理センター（犬）** `tokushima/tokushima-pref-dogs`
   - iframe内テーブル形式（table.f_a3）、猫と同じ構造、データ: 10匹

### 実装済み都道府県（13/47）

北海道、宮城、東京、埼玉、千葉、神奈川、富山、石川、福井、愛知、京都、大阪、兵庫、広島、福岡、香川、**徳島**、沖縄
...
- **愛媛県動物愛護センター** (pending)
  - URL: https://www.pref.ehime.jp/h25123/apc/joto.html

- **高知県動物愛護管理センター** (pending)
  - URL: https://www.pref.kochi.lg.jp/soshiki/131601/jyoutoneko.html
  - 高知市中央小動物管理センター（市レベル）

進捗: 13都道府県、26施設、34スクレイパー実装済み → 四国完成で14都道府県へ
...
4. 整理は `scripts/core/cleanup-html-yaml.js` で自動化済み
5. コミットは: スクレイパーコード → データファイル（整理後）の2段階
