...
```bash
# 堺市
node scripts/scrapers/osaka/sakai-city-cats/update-yaml-from-images.js

# 横浜市
node scripts/scrapers/kanagawa/yokohama-city-cats/update-yaml-from-images.js
```

### 注意事項

- **新しい猫が追加された場合**: 画像が自動ダウンロードされるが、extractedData への追加は手動
- **既存の猫が譲渡された場合**: 自動的に削除される（問題なし）
- **月1回程度**: extractedData を最新化する運用を推奨
