...
const CONFIG = {
  ...
  source_url: 'https://zuttoissho.com/omukae/animal/cat/',
};
...
/**
 * リストから猫情報を抽出
 * HTMLパターン: <a href="/omukae/animal/cat/XXXX/">
 *   c4780【ミルクボランティア猫・仮名：カイ　申込者あり】
 *   1歳 雑種 黒
 *   去勢オス
 *   東部動物愛護管理センター
 * </a>
...
function extractCatInfoFromList($) {
  ...
  $('a').each((index, elem) => {
    ...
    // c番号で始まる猫情報をフィルタリング
    const catIdMatch = linkText.match(/c(\d+)【(.+?)】/);
    if (!catIdMatch) return;

    const catNumber = catIdMatch[1]; // c後の数字
    const titleText = catIdMatch[2]; // 【】内のテキスト

    // 管理番号とexternal_id
    const managementNumber = `c${catNumber}`;
    const external_id = `fukuoka-city-${managementNumber}`;

    // 【】内から仮名と申込者ありステータスを抽出
    // 例：「ミルクボランティア猫・仮名：カイ　申込者あり」
    let name = null;
...
async function main() {
  ...
  console.log('🐱 福岡市動物愛護管理センター（猫） - YAML抽出');
  console.log('='.repeat(60) + '\n');
    ...
    const yamlContent = yaml.dump(
      {
        meta: {
          ...
          note: '福岡市譲渡猫情報（外部サイトzuttoissho.comより）',
        },
        animals: allCats,
      },
