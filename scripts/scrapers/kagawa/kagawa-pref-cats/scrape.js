...
const CONFIG = {
  ...
  pdfLinkPattern: '/documents/.*cat\\.pdf',
  timeout: 30000,
  waitTime: 3000,
};
...
async function main() {
  ...
  console.log('🐱 香川県動物愛護管理センター - PDF収集（猫）');
  console.log('='.repeat(60));
  ...
  try {
    ...
    // PDFリンクを取得
    console.log('🔍 猫用PDFリンクを検索中...');
    const pdfLinks = await page.$$eval('a[href*="cat.pdf"]', (links) =>
      links.map((link) => link.getAttribute('href'))
    );

    if (pdfLinks.length === 0) {
      throw new Error('猫用PDFリンクが見つかりませんでした');
    }
