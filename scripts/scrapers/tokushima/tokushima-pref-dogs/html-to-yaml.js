...
const CONFIG = {
  municipality: 'tokushima/tokushima-pref-dogs',
  municipalityId: null, // TODO: DB登録後に設定
  base_url: 'https://douai-tokushima.com',
  source_url: 'https://douai-tokushima.com/animalinfo/list4_1',
};
...
async function main() {
  ...
  console.log('🐶 徳島県動物愛護管理センター - YAML抽出（犬）');
  console.log('='.repeat(60) + '\n');
  ...
  try {
    ...
    // 犬情報を抽出
    console.log('🔍 犬情報を抽出中...');
    const dogs = extractDogInfoFromList($);

    console.log(`✅ ${dogs.length}匹の犬情報を抽出しました`);
    ...
    // YAML保存
    const yamlContent = yaml.dump(dogs, { indent: 2, lineWidth: -1 });
    ...
    dogs.forEach((dog, i) => {
      console.log(
        `  ${i + 1}. ${dog.name} (${dog.gender}, 生年月日: ${dog.birth_estimate || 'unknown'}, status: ${dog.status})`
      );
    });
