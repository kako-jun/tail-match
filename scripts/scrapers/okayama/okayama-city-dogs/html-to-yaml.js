...
/**
 * 岡山市保健所 犬用 HTML→YAML変換
 *
 * ページ構造:
 * - h3タグで各犬を区切り（例：1D2025123(甘平くん)）
...
const CONFIG = {
  ...
  sourceUrl: 'https://www.city.okayama.jp/kurashi/0000016441.html',
};
...
async function parseHTML() {
  ...
  console.log('📝 岡山市保健所（犬） - HTML→YAML変換');
  console.log('='.repeat(60) + '\n');
  ...
  $('h3').each((_, elem) => {
    ...
    // 管理番号パターン（1D番号）
    const idMatch = h3Text.match(/1D(\d+)/);
    if (!idMatch) return; // 管理番号がない場合はスキップ

    const managementId = `1D${idMatch[1]}`;
