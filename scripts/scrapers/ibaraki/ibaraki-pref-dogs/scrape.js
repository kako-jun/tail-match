...
/**
 * 茨城県動物指導センター スクレイピングスクリプト（犬）
 *
 * - PDF形式で公開されている犬の情報をダウンロード
 * - PDFファイル名は日付ベース（例: inu1113.pdf）
 */
...
const CONFIG = {
  municipality: 'ibaraki/ibaraki-pref-dogs',
...
async function main() {
  ...
  console.log('🐕 茨城県動物指導センター - PDF取得（犬）');
  console.log('='.repeat(60));
  ...
  try {
    ...
    const pdfLink = await page.evaluate(() => {
      ...
      const dogLink = links.find((a) => a.textContent.includes('成犬') && a.href.endsWith('.pdf'));
      return dogLink ? dogLink.href : null;
    });

    if (!pdfLink) {
      throw new Error('犬のPDFリンクが見つかりません');
    }
    ...
    const filename = `${timestamp}_dogs.pdf`;
