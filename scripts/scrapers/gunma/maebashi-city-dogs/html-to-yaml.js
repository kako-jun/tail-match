...
/**
 * 前橋市保健所（犬） YAML抽出スクリプト
 *
 * 特徴:
 * - テーブル形式（管理番号 D2025-XXX）
 * - 各犬が独立したテーブル要素
 * - caption: 管理番号 or "譲渡決定"
 * - 5行構成: [名前,品種,ワクチン] [性別,毛色] [年齢,性格] [説明] [画像]
 */
...
const CONFIG = {
  municipality: 'gunma/maebashi-city-dogs',
  base_url: 'https://www.city.maebashi.gunma.jp',
  source_url: 'https://www.city.maebashi.gunma.jp/soshiki/kenko/eiseikensa/gyomu/1/1/3/17337.html',
};
...
function extractDogs($) {
  ...
  $('table[border="1"]').each((i, table) => {
    ...
    if (!caption.match(/D2025-\d+/) && !caption.includes('譲渡')) {
      return;
    }
    ...
    const managementNumber = caption.match(/D2025-\d+/) ? caption : null;
    ...
    // Row 1: [性別, 毛色] ← 犬は猫と順序が逆
    const row1 = $(rows[1]);
    const genderText = row1.find('td').eq(0).text().trim();
    const color = row1.find('td').eq(1).text().trim();
  ...
  try {
    ...
    if (dogs.length === 0) {
      ...
      dogs.forEach((dog, index) => {
        const statusMark = dog.status === 'adopted' ? '【譲渡済】' : '';
        console.log(
          `   ${index + 1}. ${dog.name} ${statusMark}(${dog.external_id || '管理番号なし'}) - ${dog.breed}, ${dog.gender === 'male' ? 'オス' : dog.gender === 'female' ? 'メス' : '不明'}, ${dog.age_estimate}`
        );
      });
    }
