...
/**
 * 群馬県動物愛護センター HTML収集スクリプト（犬）
 *
 * URL: https://www.pref.gunma.jp/page/5761.html
 */
...
async function main() {
  ...
  console.log('🐕 群馬県動物愛護センター（犬） - HTML収集');
  console.log('='.repeat(60));
...
function countAnimalsInHTML(html) {
  // h4見出しで犬をカウント（管理番号付き）
  const h4Pattern = /<h4[^>]*>[\s\S]*?（[^）]+）[\s\S]*?<\/h4>/gi;
  ...
  // フォールバック: 管理番号パターンで検出（犬は年度不明のため汎用）
  const idPattern = /（[A-Z]-\d+-\d+）/gi;
