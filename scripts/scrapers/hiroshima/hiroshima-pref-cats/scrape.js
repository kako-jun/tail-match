...
/**
 * 広島県動物愛護センター（猫） HTML収集スクリプト
 *
 * URL: https://www.pref.hiroshima.lg.jp/site/apc/jouto-rebreed-cat-list.html
 */
...
async function main() {
  ...
  console.log('🐱 広島県動物愛護センター（猫） - HTML収集');
  console.log('='.repeat(60));
...
/**
 * HTML内の動物数をカウント
 * 広島県は<h2>タグで各猫を識別（例: 管理番号：1HN20250139(145番)）
 */
function countAnimalsInHTML(html) {
  // パターン1: <h2>管理番号：1HN番号(表示番号)</h2>
  const h2Pattern = /<h2[^>]*>管理番号：1HN\d+[（(]\d+番[)）]/gi;
