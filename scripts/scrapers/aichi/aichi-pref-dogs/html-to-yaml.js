...
/**
 * 愛知県動物愛護センター（全支所統合）犬 YAML抽出スクリプト
 */
 ...
 * テーブル行から犬情報を抽出
 */
function extractDogFromTableRow($, $row, branchName) {
  ...
  // 品種を抽出
  const breedMatch = detailText.match(/(雑種|柴犬|チワワ|プードル|ミックス)/);
  ...
  try {
    ...
    for (let i = 1; i < branches.length; i += 2) {
      ...
      $('table tr').each((index, row) => {
        ...
        const dog = extractDogFromTableRow($, $row, branchName);
        if (dog) {
          allDogs.push(dog);
          console.log(`   犬 ${allDogs.length}: ${dog.external_id}`);
          console.log(`      性別: ${dog.gender}, 年齢: ${dog.age_estimate || '不明'}`);
          console.log(`      品種: ${dog.breed || '不明'}, 状態: ${dog.status}`);
        }
      });
    }
