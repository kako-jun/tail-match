...
/**
 * 京都府動物愛護管理センター スクレイパー (Playwright版)
 *
 * 特徴:
...
const CONFIG = {
  municipality: 'kyoto/kyoto-pref',
  url: 'https://kyoto-ani-love.com/recruit-animal/cat/',
  expected_selectors: 'div.content.clearfix',
...
async function fetchWithRetry(url, retries = CONFIG.retry_count) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      // 動的コンテンツの読み込み完了を確認
      try {
        await page.waitForSelector('div.content.clearfix', {
          timeout: 5000,
        });
...
async function main() {
  ...
  console.log('🐱 京都府動物愛護管理センター - HTML収集 (Playwright版)');
  console.log('='.repeat(60));
