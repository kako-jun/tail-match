...
INDEX_URL="https://www.city.sakai.lg.jp/kurashi/dobutsu/dogdog/inunekojoto/index.html"
PAGE_URLS=$(curl -k -s "$INDEX_URL" | \
  grep -o 'href="[^"]*\(dogs\|centerdogs\)[^"]*\.html"' | \
  sed 's/href="//;s/"$//' | \
  sort -u)
...
echo "$PAGE_URLS" | while read -r rel_url; do
  ...
  # ファイル名決定
  if echo "$rel_url" | grep -q "dogs1"; then
    filename="${timestamp}_dogs1.html"
  elif echo "$rel_url" | grep -q "dogs2"; then
    filename="${timestamp}_dogs2.html"
  elif echo "$rel_url" | grep -q "dogs3"; then
    filename="${timestamp}_dogs3.html"
...
IMAGE_URLS=$(grep -oh '\(dogs[0-9]\|centerdogs\)\.images/[^"]*\.png' "$HTML_DIR"/*.html 2>/dev/null | sort -u || true)
...
echo "$IMAGE_URLS" | while read -r img_path; do
  if [ -n "$img_path" ]; then
    ...
    full_url="https://www.city.sakai.lg.jp/kurashi/dobutsu/dogdog/inunekojoto/${img_path}"
