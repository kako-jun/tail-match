...
/**
 * 新潟市動物愛護センター HTML収集スクリプト（犬）
 *
 * URL: https://www.ikutopia.com/facilities/doubutsu/foster/dog/
...
const CONFIG = {
  ...
  url: 'https://www.ikutopia.com/facilities/doubutsu/foster/dog/',
  expected_selectors: 'article, .dog-card, a[href*="/dog/"]',
  timeout: 30000,
...
async function main() {
  ...
  console.log('🐕 新潟市動物愛護センター - HTML収集（犬）');
  console.log('='.repeat(60));
  ...
  try {
    ...
    const metadata = {
      ...
      scraper: 'playwright',
      note: 'カード型レイアウト、管理番号（2025d10など）・名前・年齢表示',
    };
...
/**
 * HTML内の動物数をカウント
 * 新潟市（いくとぴあ）は管理番号（2025d10など）で各犬を識別
 */
function countAnimalsInHTML(html) {
  // 管理番号パターン（例：2025d10）を検索
  const pattern = /\d{4}d\d+/g;
