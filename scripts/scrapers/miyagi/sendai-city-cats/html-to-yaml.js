...
/**
 * 仙台市動物管理センター「アニパル仙台」（猫） YAML抽出スクリプト
 */
...
const CONFIG = {
  ...
  source_url: 'https://www.city.sendai.jp/dobutsu/kurashi/shizen/petto/hogodobutsu/joho/neko.html',
};
    ...
    if (caption.includes('譲渡対象猫の情報')) {
      ...
      $table.find('tbody > tr').each((rowIndex, tr) => {
        ...
        cats[managementNumber] = {
          ...
          extraction_notes: ['譲渡対象猫'],
          listing_type: 'adoption',
        };
      });
    } else if (caption.includes('譲渡猫の写真')) {
      // 写真テーブル処理
      $table.find('tbody > tr').each((rowIndex, tr) => {
        const $tr = $(tr);
        const $tds = $tr.find('td');
...
async function main() {
  ...
  console.log('🐱 仙台市動物管理センター「アニパル仙台」（猫） - YAML抽出');
  console.log('='.repeat(60) + '\n');
